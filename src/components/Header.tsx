      ...
      <header className={`fixed top-[var(--top-banner-height)] left-0 z-[1000] w-full h-[var(--header-height)] bg-brand-black flex items-center border-b border-white/10 backdrop-blur-md transition-colors duration-300 ease-in-out ${isScrolled ? 'shadow-md' : ''}`} id="header"> {/* Apply shadow class based on state */}
        ...
        <div className="w-full max-w-screen-xl mx-auto px-4">
          <nav className="flex justify-between items-center w-full">
            ...
            {/* TODO: Add dropdown hover logic */}
            <ul className="hidden lg:flex gap-x-6 xl:gap-x-10 items-center mx-auto flex-nowrap"> {/* Removed overflow-hidden */}
              <li className="relative group"> {/* Added group for dropdown hover */}
                <Link href="/shop" className="text-white/85 font-normal text-[0.85rem] uppercase tracking-wider relative pb-2 transition-colors duration-200 hover:text-white whitespace-nowrap flex items-center group-hover:text-white after:content-[''] after:absolute after:w-0 after:h-[1px] after:bottom-0 after:left-1/2 after:transform after:-translate-x-1/2 after:bg-brand-gold after:transition-all after:duration-300 after:ease-in-out group-hover:after:w-full">
                  Shop
              ...
              <li className="relative group"> {/* Added group for dropdown hover */}
                <Link href="/account" className="text-white/85 font-normal text-[0.85rem] uppercase tracking-wider relative pb-2 transition-colors duration-200 hover:text-white whitespace-nowrap flex items-center group-hover:text-white after:content-[''] after:absolute after:w-0 after:h-[1px] after:bottom-0 after:left-1/2 after:transform after:-translate-x-1/2 after:bg-brand-gold after:transition-all after:duration-300 after:ease-in-out group-hover:after:w-full">
                  Account
                  <span className="ml-1.5 text-xs transition-transform duration-300 group-hover:rotate-180">▼</span>
                </Link>
                 {/* Desktop Dropdown */}
            ...
            <button
              ...
              onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} // Basic toggle
            >
        ...
        <div className={`lg:hidden fixed top-[calc(var(--header-height)+var(--top-banner-height))] left-0 w-full bg-brand-black py-6 flex flex-col items-center gap-0 z-[999] shadow-md transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-y-0 opacity-100 visible' : '-translate-y-full opacity-0 invisible'} max-h-[calc(100vh-var(--header-height)-var(--top-banner-height))] overflow-y-auto`}> {/* Re-added max-h and overflow-y-auto */}
          ...
          <div className="w-full text-center py-3 px-4 flex justify-center items-center flex-wrap">
            ...
            <button
              ...
              onClick={() => {
                ...
                setIsAccountDropdownOpen(false); // Close other dropdown
              }} // Toggle Shop dropdown state and close Account
            >
              {isShopDropdownOpen ? '−' : '+'} {/* Change button text based on state */}
            </button>
          </div>
          {/* Mobile Dropdown (Shop) - Conditionally show based on state */}
          <div className={`w-full bg-black/30 py-2 pl-10 mt-2 ${isShopDropdownOpen ? 'block' : 'hidden'}`}> {/* Show/hide based on state */}
            <li><Link href="/shop/collections/red-showstoppers" className="block py-2.5 text-sm text-white/70 hover:text-brand-gold" onClick={closeMobileMenu}>Red Showstoppers</Link></li> {/* Close menu on link click */}
          ...
          {/* Other Mobile Nav Items (close menu on click) */}
          ...
          <div className="w-full text-center py-3 px-4 flex justify-center items-center flex-wrap">
            <Link href="/account" className="text-white/90 text-base uppercase tracking-[1.5px] block hover:text-brand-gold" onClick={closeMobileMenu}>Account</Link> {/* Close menu on link click */}
            <button
              className="text-white/70 bg-none border-none text-xl px-2 cursor-pointer ml-2 leading-none hover:text-brand-gold"
              aria-label="Toggle Account Menu"
              onClick={() => {
                setIsAccountDropdownOpen(!isAccountDropdownOpen);
                setIsShopDropdownOpen(false); // Close other dropdown
              }} // Toggle Account dropdown state and close Shop
            >
              {isAccountDropdownOpen ? '−' : '+'} {/* Change button text based on state */}
            </button>
          </div>
           {/* Mobile Dropdown (Account) - Conditionally show based on state */}
          <div className={`w-full bg-black/30 py-2 pl-10 mt-2 ${isAccountDropdownOpen ? 'block' : 'hidden'}`}> {/* Show/hide based on state */}
            <li><Link href="/account/login" className="block py-2.5 text-sm text-white/70 hover:text-brand-gold" onClick={closeMobileMenu}>Login</Link></li> {/* Close menu on link click */}
